 ...
 * promise is not free: `wp_delete_attachment()` destroys the file immediately
 * and permanently unless the site has `MEDIA_TRASH` defined.
 *
 * So the plugin never calls it for a removal — it trashes the post directly,
 * which works whatever the site is configured to do. But **recovery must not
 * depend on that either** — a hosting layer can intervene, a future WordPress
 * can change the semantics. So before anything is touched, the full record is
 * written here: the post row, every meta row, the attached file, and every
 * generated size on disk.
 *
...
final class RestoreManifest {
	...
	public function history_for( int $attachment_id ): array {
		...
		return (array) $wpdb->get_results(
			$wpdb->prepare(
				'SELECT action, created_at, restored_at FROM %i
