...
final class ScanCommand {
	 ...
	 *
	 *     wp janitorix scan
	 *     wp janitorix scan --fresh --breakdown
	 *     wp janitorix scan --status=possibly_used --limit=0
	 *
	...
	public function explain( array $args, array $assoc ): void { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- WP-CLI always calls a command callback with (array $args, array $assoc), whether or not the command uses either.
		...
		if ( $attachment_id <= 0 ) {
			\WP_CLI::error( 'Provide an attachment ID: wp janitorix explain <id>' );
		}
		...
		if ( null === $row ) {
			\WP_CLI::error( sprintf( 'No stored report for attachment #%d. Run `wp janitorix scan` first.', $attachment_id ) );
		}
		...
		$report->confidence_level       = \JanitorixMediaAudit\Confidence\ConfidenceEngine::level( (int) $row->confidence );
		...
		foreach ( $repo->references_for( (int) $row->scan_id, $attachment_id ) as $ref ) {
			$report->evidence[] = new \JanitorixMediaAudit\Scanner\Reference(
				$attachment_id,
				$ref->scanner,
				$ref->location_type,
				(int) $ref->location_id,
		...
		foreach ( ( new \JanitorixMediaAudit\Reports\ExplanationBuilder() )->lines( $report ) as $line ) {
			\WP_CLI::log( $line );
		}
	}
	...
	public function calibrate( array $args, array $assoc ): void { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed -- WP-CLI always calls a command callback with (array $args, array $assoc), whether or not the command uses either.
		...
		$run = ( new \JanitorixMediaAudit\Calibration\CalibrationRun() )->run();
	 ...
	 * ## EXAMPLES
	 *
	 *     wp janitorix would-delete
	 *     wp janitorix would-delete --csv > candidates.csv
	 *
	 * @param string[]             $args  Positional arguments (unused).
	 * @param array<string,string> $assoc The parsed --flags.
	 *
	...
	public function would_delete( array $args, array $assoc ): void {
		$controller = Plugin::instance()->controller();
		$report     = new \JanitorixMediaAudit\Calibration\BetaReport(
		...
		if ( empty( $data['available'] ) ) {
			\WP_CLI::error( 'No scan yet. Run `wp janitorix scan` first.' );
		}

		\WP_CLI::log( sprintf( 'Scan #%d · %d%% coverage', $data['scan_id'], $data['coverage'] ) );

		if ( ! empty( $data['stale'] ) ) {
			\WP_CLI::warning( 'The site has changed since this scan. Run `wp janitorix scan --fresh` before asking anyone to check this list.' );
		}
		...
		foreach ( $rows as $row ) {
			\WP_CLI::log(
				sprintf(
					...
					$row->attachment_id,
					$row->confidence,
					\JanitorixMediaAudit\Confidence\ConfidenceEngine::level( (int) $row->confidence ),
					$row->filename
				)
			);
		}
