...
namespace JanitorixMediaAudit\Recommendation;
...
final class RecommendationEngine {
	...
	private function decide_one( ImageReport $report ): void {
		...
		if ( ImageReport::STATUS_USED === $report->status ) {
			$this->set( $report, self::KEEP, array( __( 'Verified in use.', 'janitorix-media-audit' ), $this->confidence_line( $report ) ) );
		...
		if ( ImageReport::STATUS_POSSIBLY_USED === $report->status ) {
			$this->set(
				$report,
				self::REVIEW,
				array( __( 'A weak reference was found — not enough to prove use, enough to stop a deletion.', 'janitorix-media-audit' ) )
			);
		...
		if ( $report->coverage < ConfidenceEngine::COVERAGE_FLOOR ) {
			$this->set(
				...
				array(
					...
					__( 'A clean result on a partial scan is not a result.', 'janitorix-media-audit' ),
				)
			);
		...
		if ( $risk >= RiskEngine::CRITICAL ) {
			$this->set(
				...
				self::KEEP,
				array_merge(
					array( __( 'Critical risk: deleting this would break the site.', 'janitorix-media-audit' ) ),
					$report->risk_impact
				)
			);
		...
		if ( $risk >= RiskEngine::HIGH ) {
					...
					array_merge(
						array(
							sprintf(
								...
								__( 'High risk and only %1$d%% confidence (%2$s) — too little certainty about too valuable an image.', 'janitorix-media-audit' ),
								$confidence,
			...
			$this->set(
				...
				self::REVIEW,
				array_merge(
					array( sprintf( /* translators: %s: risk level */ __( '%s risk — a human should look before this is deleted.', 'janitorix-media-audit' ), $report->risk_level ) ),
					$report->risk_impact
				)
			);
			...
			$this->set(
				...
				array(
					sprintf(
						...
						__( '%1$d%% confidence (%2$s) — below the %3$d%% needed to recommend removal.', 'janitorix-media-audit' ),
						$confidence,
						$report->confidence_level,
						ConfidenceEngine::HIGH
					),
					__( 'Nothing was found, but not enough of the site was searched well enough to say so.', 'janitorix-media-audit' ),
					sprintf(
					/* translators: %d: percentage of the site searched */
						__( '%d%% scan coverage.', 'janitorix-media-audit' ),
						$report->coverage
					),
				)
			);
		...
		$this->set(
			...
			self::MOVE_TO_TRASH,
			array(
				__( 'No references found.', 'janitorix-media-audit' ),
				sprintf( /* translators: %s: risk level */ __( '%s risk.', 'janitorix-media-audit' ), $report->risk_level ),
		...
		switch ( $report->user_decision ) {
			case UserDecisions::EXCLUDED:
				$this->set(
					...
					self::EXCLUDED,
					array(
						__( 'You excluded this image from scans.', 'janitorix-media-audit' ),
						__( 'It is still scanned so it can be found again, and it is never recommended for anything. Remove the exclusion to bring it back.', 'janitorix-media-audit' ),
					)
				);
			...
			case UserDecisions::SAFE:
				$this->set(
					...
					self::MARK_SAFE,
					array(
						__( 'You marked this image as in use.', 'janitorix-media-audit' ),
						__( 'No scan will treat it as unused while that stands.', 'janitorix-media-audit' ),
					)
				);
			...
			case UserDecisions::IGNORE:
				$this->set(
					...
					self::IGNORE,
					array(
						__( 'You asked to skip this image during reviews.', 'janitorix-media-audit' ),
						__( 'It stays in the media library and keeps being scanned — it is simply left out of the lists.', 'janitorix-media-audit' ),
					)
				);
	...
	private function confidence_line( ImageReport $report ): string {
		return sprintf(
			...
			__( '%1$d%% confidence (%2$s).', 'janitorix-media-audit' ),
			$report->confidence,
			$report->confidence_level
		);
	}
