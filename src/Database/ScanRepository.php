...
namespace JanitorixMediaAudit\Database;
...
use JanitorixMediaAudit\Scanner\ScannerState;
...
final class ScanRepository {
	...
	public function start( string $fingerprint ): int {
		...
		$wpdb->insert(
			...
			array(
				...
				'plugin_version'      => JANITORIX_VERSION,
	...
	public function complete( int $scan_id, array $reports, ConfidenceEngine $confidence, array $scanner_states, int $duration_ms, array $broken = array() ): void {
		...
		$wpdb->update(
			...
			array(
				...
				'absence_confidence'   => $confidence->absence_confidence(),
			...
			array( 'id' => $scan_id ),
			array( '%s', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%s', '%s', '%s' ),
			array( '%d' )
		);
	}
	...
	private function insert_reports( int $scan_id, array $reports ): void {
		...
		$tuple = '(%d,%d,%s,%s,%d,%d,%d,%d,%s,%s,%s,%s,%s,%s,%d,%d)';
		...
		foreach ( $reports as $report ) {
			...
			if ( count( $batch ) >= self::INSERT_CHUNK ) {
				$this->flush( $table, $columns, $tuple, $batch );
				$batch = array();
			}
		}
	 ...
	 *
	 * The whole statement — table, every column name, and every value — goes
	 * through one `prepare()` call. Nothing is concatenated in: the table and
	 * the columns are `%i` identifier placeholders, and the values fill the
	 * repeated tuple. The only thing built by hand is the run of placeholders
	 * itself, which contains no data.
	 *
	 * @param string   $table   The table to insert into.
	 * @param string[] $columns Column names, in the same order as the tuple.
	 * @param string   $tuple   One row's placeholder tuple, e.g. "(%d,%s)".
	 * @param array[]  $rows    One array of values per row.
	 */
	private function flush( string $table, array $columns, string $tuple, array $rows ): void {
		if ( empty( $rows ) ) {
			return;
		}
		...
		$column_list = implode( ',', array_fill( 0, count( $columns ), '%i' ) );
		$values      = implode( ',', array_fill( 0, count( $rows ), $tuple ) );

		$args = array_merge( array( $table ), $columns );

		foreach ( $rows as $row ) {
			$args = array_merge( $args, array_values( $row ) );
		}

		// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $column_list and $values are placeholder runs generated above, one entry per column and per value, and every one of them is filled from $args in this same call. They carry no data.
		$wpdb->query(
			$wpdb->prepare( "INSERT INTO %i ({$column_list}) VALUES {$values}", $args )
		);
	...
	public function latest_running( int $scan_id ) {
		...
		return $wpdb->get_row(
			$wpdb->prepare( 'SELECT * FROM %i WHERE id = %d', Tables::scans(), $scan_id )
		);
	}
	...
	public function latest() {
		...
		return $wpdb->get_row(
			$wpdb->prepare( "SELECT * FROM %i WHERE status = 'completed' ORDER BY id DESC LIMIT 1", Tables::scans() )
		);
	}
	...
	public function previously_used( int $before_scan_id ): array {
		...
		$previous = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT id FROM %i WHERE status = 'completed' AND id < %d ORDER BY id DESC LIMIT 1",
				Tables::scans(),
				$before_scan_id
			)
		);
	...
	public function unreferenced_with_good_history( int $before_scan_id ): array {
		...
		$scans = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT id, completed_at, scanner_states FROM %i
	...
	public static function result_of( object $scan ): string {
		if ( 'failed' === $scan->status ) {
			return __( 'Failed', 'janitorix-media-audit' );
		}

		if ( 'completed' !== $scan->status ) {
			return __( 'Running', 'janitorix-media-audit' );
		}
		...
		foreach ( $states as $state ) {
			if ( in_array( $state, array( 'failed', 'not_implemented' ), true ) ) {
				return __( 'Partially Completed', 'janitorix-media-audit' );
			}
		}

		foreach ( $states as $state ) {
			if ( 'warning' === $state ) {
				return __( 'Completed With Warnings', 'janitorix-media-audit' );
			}
		}

		return __( 'Completed', 'janitorix-media-audit' );
	}
	...
	public function reports( int $scan_id, ?string $status = null, int $limit = 0 ): array {
		...
		$sql  = 'SELECT * FROM %i WHERE scan_id = %d';
		$args = array( Tables::reports(), $scan_id );
		...
		// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- $sql is assembled a few lines above from literal fragments only; every value it needs is in $args and goes through prepare() here. The sniff cannot trace a query string across statements.
	...
	public function browse( int $scan_id, array $filters = array(), int $page = 1, int $per_page = 25 ): array {
		...
		// The two table identifiers sit at the very start of the statement, so
		// their arguments lead the list; everything the filters add follows in
		// the order the clauses were appended.
		$args = array( Tables::reports(), $wpdb->posts, $scan_id );

		// The column is never built from the filter name. `%i` cannot help here —
		// it would quote "r.status" as one identifier — so each allowed column
		// is written out as a literal fragment and only the value is a
		// placeholder. A filter key that is not in this map cannot reach SQL.
		foreach ( array(
		...
		if ( ! empty( $filters['scanner'] ) ) {
			$where[] = 'EXISTS ( SELECT 1 FROM %i ref
				WHERE ref.scan_id = r.scan_id AND ref.attachment_id = r.attachment_id AND ref.scanner = %s )';
			$args[]  = Tables::references();
			$args[]  = $filters['scanner'];
		}

		// The comparison operator is written out on both branches rather than
		// interpolated: an operator cannot be a placeholder, so the only way it
		// is provably not data is for it to be literal in the source.
		...
		$from   = "FROM %i r LEFT JOIN %i p ON p.ID = r.attachment_id WHERE {$clause}";

		// phpcs:ignore WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $from is built above from literal SQL and '%i'/'%d'/'%s' placeholders only, never from a filter value; $args is built alongside $where, one entry per placeholder, in the same order.
		$total = (int) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) {$from}", $args ) );

		$offset = max( 0, ( $page - 1 ) * $per_page );

		// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- array_merge() appends exactly the 2 values for the trailing LIMIT %d OFFSET %d on top of $args, which already matches every placeholder embedded in $from.
	...
	public function history( int $limit = 25 ): array {
		...
		return (array) $wpdb->get_results(
			$wpdb->prepare(
				'SELECT * FROM %i WHERE completed_at IS NOT NULL ORDER BY id DESC LIMIT %d',
				Tables::scans(),
				$limit
			)
		);
	...
	public function recoverable_bytes( array $scan_ids ): array {
		...
		// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $placeholders is a run of literal '%d', one per scan id, filled from the merged args of this same prepare() call.
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT scan_id, SUM( CASE WHEN recommendation = %s THEN filesize ELSE 0 END ) AS recoverable
				 ...
				 GROUP BY scan_id",
				array_merge( array( 'move_to_trash', Tables::reports() ), $scan_ids )
			)
		);
		// phpcs:enable WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.InterpolatedNotPrepared
	...
	private function prune_to( int $keep ): void {
		...
		$wpdb->query( $wpdb->prepare( 'DELETE FROM %i WHERE scan_id <= %d', Tables::references(), $cutoff ) );
		$wpdb->query( $wpdb->prepare( 'DELETE FROM %i WHERE scan_id <= %d', Tables::reports(), $cutoff ) );
		$wpdb->query( $wpdb->prepare( 'DELETE FROM %i WHERE scan_id <= %d', Tables::logs(), $cutoff ) );
		$wpdb->query( $wpdb->prepare( 'DELETE FROM %i WHERE id <= %d', Tables::scans(), $cutoff ) );
	}
}
