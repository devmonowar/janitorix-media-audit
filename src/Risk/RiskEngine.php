...
namespace JanitorixMediaAudit\Risk;
...
final class RiskEngine {
	...
	private function measure( ImageReport $report ): void {
		...
		$report->risk_breakdown = array(
			array(
				'signal' => sprintf( /* translators: %s: where the image is used */ __( 'worst location: %s', 'janitorix-media-audit' ), $worst_label ),
				'effect' => 'impact',
				'points' => $highest,
			),
			array(
				'signal' => sprintf( /* translators: %d: number of published locations */ _n( '%d published location', '%d published locations', $published, 'janitorix-media-audit' ), $published ),
				'effect' => 'breadth',
				'points' => $escalation,
			),
		);
	...
	private function infer( ImageReport $report ): void {
		...
		$floor       = self::BASELINE;
		$breakdown[] = array(
			'signal' => __( 'baseline (unreferenced)', 'janitorix-media-audit' ),
			'effect' => 'base',
		...
		if ( $furniture ) {
			$floor       = max( $floor, 60 );
			$breakdown[] = array(
				'signal' => __( 'site-furniture profile', 'janitorix-media-audit' ),
				'effect' => 'floor',
				'points' => 60,
			);
		}
		...
		if ( $report->outside_uploads ) {
			$floor       = max( $floor, 70 );
			$breakdown[] = array(
				'signal' => __( 'lives outside the uploads folder', 'janitorix-media-audit' ),
				'effect' => 'floor',
				'points' => 70,
			);
		}
		...
		if ( $report->stopped_being_used ) {
			$floor       = max( $floor, 50 );
			$breakdown[] = array(
				'signal' => __( 'was in use at the last scan, and the place that used it is still there', 'janitorix-media-audit' ),
				'effect' => 'floor',
				'points' => 50,
			);
		}
		...
		if ( in_array( 'customizer', $this->failed_scanners, true ) && $this->matches_site_identity_option( $report ) ) {
			$floor       = max( $floor, 100 );
			$breakdown[] = array(
				'signal' => __( 'registered in a WordPress core option, and the Customizer scanner failed', 'janitorix-media-audit' ),
				'effect' => 'floor',
				'points' => 100,
			);
		}
		...
		if ( $this->is_plausible_social_card( $report ) ) {
			$additions  += 15;
			$breakdown[] = array(
				'signal' => __( 'plausible social / OG card', 'janitorix-media-audit' ),
				'effect' => 'add',
				'points' => 15,
			);
		}
		...
		if ( $report->has_curated_meta ) {
			$additions  += 10;
			$breakdown[] = array(
				'signal' => __( 'curated metadata (alt / caption)', 'janitorix-media-audit' ),
				'effect' => 'add',
				'points' => 10,
			);
		}
		...
		if ( $report->is_svg ) {
			$additions  += 10;
			$breakdown[] = array(
				'signal' => __( 'SVG — almost never a content photo', 'janitorix-media-audit' ),
				'effect' => 'add',
				'points' => 10,
			);
		}
		...
		if ( '' !== $blind ) {
			$additions  += 20;
			$breakdown[] = array(
				'signal' => sprintf( /* translators: %s: scanner name */ __( 'the %s scanner failed and would have covered this image', 'janitorix-media-audit' ), $blind ),
				'effect' => 'add',
				'points' => 20,
			);
		}
		...
		if ( ( $this->duplicate_counts[ $report->attachment_id ] ?? 0 ) > 0 ) {
			...
			$attenuation += 20;
			$breakdown[]  = array(
				'signal' => __( 'byte-identical duplicate exists elsewhere in the library', 'janitorix-media-audit' ),
				'effect' => 'subtract',
				'points' => -20,
			);
		}
		...
		if ( $report->parent_deleted && ( $this->stranded_siblings_unused[ $report->attachment_id ] ?? false ) ) {
			...
			$attenuation += 10;
			$breakdown[]  = array(
				'signal' => __( 'parent post deleted, and its sibling uploads are also unused', 'janitorix-media-audit' ),
				'effect' => 'subtract',
				'points' => -10,
			);
		}
		...
		if ( $report->unreferenced_with_good_history ) {
			...
			$attenuation += 10;
			$breakdown[]  = array(
				'signal' => __( 'unreferenced across two scans, 30+ days apart, with no scanner failures', 'janitorix-media-audit' ),
				'effect' => 'subtract',
				'points' => -10,
			);
		}
	...
	private function measured_impact_statement( ImageReport $report, int $highest, int $published ): array {
		...
		$lines[] = sprintf( __( 'If deleted, this breaks: %s.', 'janitorix-media-audit' ), $worst );
		...
		if ( $report->width > 0 && $report->height > 0 ) {
			$lines[] = sprintf(
				...
				__( 'Its %1$d×%2$d generated sizes are destroyed with it.', 'janitorix-media-audit' ),
				$report->width,
				$report->height
			);
		}
	...
	private function inferred_impact_statement( ImageReport $report, bool $furniture ): array {
		if ( $furniture ) {
			return array(
				__( 'No references were found — but the filename matches the site-furniture profile.', 'janitorix-media-audit' ),
				__( 'Site furniture hides in theme options and templates, the least searchable part of the site.', 'janitorix-media-audit' ),
				__( 'We cannot tell you this is safe to delete. We can only tell you we did not find it.', 'janitorix-media-audit' ),
			);
		}

		if ( $report->outside_uploads ) {
			return array(
				__( 'No references were found — but this file lives outside the uploads folder.', 'janitorix-media-audit' ),
				__( 'It was shipped by a theme or plugin, which means something expects it to be there.', 'janitorix-media-audit' ),
			);
		}

		return array( __( 'No references found, and nothing about the file suggests it is site furniture.', 'janitorix-media-audit' ) );
	}
