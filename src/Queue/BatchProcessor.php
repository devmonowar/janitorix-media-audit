...
namespace JanitorixMediaAudit\Queue;

use JanitorixMediaAudit\Analysis\AnalysisEngine;
...
final class BatchProcessor {
	 ...
	 * @param array<string,mixed> $state The accumulated resume state.
	 *
	 * @return \JanitorixMediaAudit\Scanner\ScannerResult[]
	 */
		...
		foreach ( $state['scanner_states'] as $id => $scanner_state ) {
			...
			switch ( $scanner_state ) {
				case ScannerState::SUCCESS:
					$results[ $id ] = \JanitorixMediaAudit\Scanner\ScannerResult::success( $id, $refs, $examined );
					break;
				case ScannerState::WARNING:
					$results[ $id ] = \JanitorixMediaAudit\Scanner\ScannerResult::warning( $id, $refs, 'see log', $examined );
					break;
				case ScannerState::SKIPPED:
					$results[ $id ] = \JanitorixMediaAudit\Scanner\ScannerResult::skipped( $id, 'not applicable' );
					break;
				case ScannerState::NOT_IMPLEMENTED:
					$results[ $id ] = \JanitorixMediaAudit\Scanner\ScannerResult::not_implemented( $id );
					break;
				default:
					...
					$results[ $id ] = \JanitorixMediaAudit\Scanner\ScannerResult::failed( $id, 'see log', $refs, $examined );
			}
	 ...
	 *
	 * @param \JanitorixMediaAudit\Scanner\Reference $r The reference to pack.
	 */
	private function pack( \JanitorixMediaAudit\Scanner\Reference $r ): array {
		return array(
	 ...
	 */
	private function unpack( array $p ): \JanitorixMediaAudit\Scanner\Reference {
		return new \JanitorixMediaAudit\Scanner\Reference(
			$p['attachment_id'],
			$p['scanner'],
			$p['location_type'],
			$p['location_id'],
