...
use JanitorixMediaAudit\Cleanup\RestoreManifest;
...
use JanitorixMediaAudit\Reports\ExplanationBuilder;
use JanitorixMediaAudit\Reports\ImageReport;
use JanitorixMediaAudit\Safety\SafetyEngine;
use JanitorixMediaAudit\Scanner\Reference;
...
final class ImageDetailsPage {
	...
	public function render(): void {
		...
		if ( null === $row ) {
			echo '<h1>' . esc_html__( 'Image Details', 'janitorix-media-audit' ) . '</h1>';
			echo '<div class="janitorix-empty"><p>' . esc_html__( 'No stored report for this image. Run a scan first.', 'janitorix-media-audit' ) . '</p></div></div>';
		...
		echo '<div class="janitorix-panel">';
	...
	private function result_notice(): void {
		...
		printf(
			...
			'ok' === $_GET['janitorix_result'] ? 'success' : 'error',
	...
	private function actions( int $attachment_id ): void {
		...
		echo '<div class="janitorix-panel" style="border-top:1px solid #c3c4c7;margin-bottom:16px">';

		if ( 'trash' === $status ) {
			$this->action_button( $attachment_id, 'restore', __( 'Restore', 'janitorix-media-audit' ), 'button-primary' );
			...
			if ( $verdict->allowed() ) {
				...
				$this->action_button(
					$attachment_id,
					'delete',
					__( 'Delete permanently', 'janitorix-media-audit' ),
					'button-link-delete',
					__( 'This cannot be undone. The file and every generated size are destroyed. Continue?', 'janitorix-media-audit' )
				);

				echo '<p class="janitorix-t">' . esc_html__( 'Make sure your site has a recent backup before deleting permanently — Trash is reversible, this is not.', 'janitorix-media-audit' ) . '</p>';
			}

			echo '<p class="janitorix-t">' . esc_html__( 'This image is in the Trash. Restoring puts it back; deleting is permanent.', 'janitorix-media-audit' ) . '</p>';
		...
		if ( $verdict->allowed() ) {
			$this->action_button(
				$attachment_id,
				'trash',
				__( 'Move to Trash', 'janitorix-media-audit' ),
				'button-primary',
				__( 'Move this image to the Trash? You can restore it afterwards.', 'janitorix-media-audit' )
			);
		} else {
			// A refusal is information, not a dead end. Say which rule fired.
			echo '<p><strong>' . esc_html__( 'This image cannot be moved to Trash.', 'janitorix-media-audit' ) . '</strong></p>';
			echo '<ul class="janitorix-reasons">';
	...
	private function decision_controls( int $attachment_id ): void {
		...
		echo '<p><strong>' . esc_html__( 'Your decision', 'janitorix-media-audit' ) . '</strong></p>';

		$labels = array(
			UserDecisions::IGNORE   => __( 'Ignore — leave it out of reviews', 'janitorix-media-audit' ),
			UserDecisions::SAFE     => __( 'Mark Safe — never treat it as unused', 'janitorix-media-audit' ),
			UserDecisions::EXCLUDED => __( 'Exclude Forever — never recommend anything for it', 'janitorix-media-audit' ),
		);

		if ( '' !== $current ) {
			printf(
				'<p><span class="janitorix-pill janitorix-neutral">%s</span></p>',
		...
		if ( '' !== $current ) {
			$this->decision_button( $attachment_id, 'clear', __( 'Clear this decision', 'janitorix-media-audit' ) );
		}

		echo '</p>';
		echo '<p class="janitorix-t">' . esc_html__( 'These outrank the scan. Marking an image safe or excluding it also stops the plugin trashing it, whatever a later scan concludes.', 'janitorix-media-audit' ) . '</p>';
	}
	...
	private function decision_button( int $attachment_id, string $decision, string $label ): void {
		...
		wp_nonce_field( 'janitorix_decide' );
		echo '<input type="hidden" name="action" value="janitorix_decide">';
		printf( '<input type="hidden" name="image" value="%d">', absint( $attachment_id ) );
	...
	private function action_button( int $attachment_id, string $action_id, string $label, string $css_class, string $confirm = '' ): void {
		// The prompt travels as a data attribute rather than an inline
		// onsubmit handler: the enqueued script binds it, so this screen emits
		// no JavaScript of its own. `confirm-field` is what tells that script
		// to stamp the hidden "confirmed" input once the person has said yes —
		// the permanent-delete handler refuses a submission without it.
		printf(
			'<form method="post" action="%s" style="display:inline"%s%s>',
			esc_url( admin_url( 'admin-post.php' ) ),
			'' === $confirm ? '' : ' data-janitorix-confirm="' . esc_attr( $confirm ) . '"',
			'delete' === $action_id ? ' data-janitorix-confirm-field="confirmed"' : ''
		);

		wp_nonce_field( 'janitorix_cleanup_' . $attachment_id );

		printf( '<input type="hidden" name="action" value="janitorix_cleanup">' );
		printf( '<input type="hidden" name="do" value="%s">', esc_attr( $action_id ) );
		printf( '<input type="hidden" name="attachment_id" value="%d">', absint( $attachment_id ) );
	...
	private function tabs( int $attachment_id, string $current ): void {
		$tabs = array(
			'overview'   => __( 'Overview', 'janitorix-media-audit' ),
			'usage'      => __( 'Usage', 'janitorix-media-audit' ),
			'analysis'   => __( 'Analysis', 'janitorix-media-audit' ),
			'timeline'   => __( 'Timeline', 'janitorix-media-audit' ),
			'simulation' => __( 'Simulation', 'janitorix-media-audit' ),
			'logs'       => __( 'Logs', 'janitorix-media-audit' ),
		);
		...
		printf( '<div class="janitorix-tabs" role="tablist" aria-label="%s">', esc_attr__( 'Image details sections', 'janitorix-media-audit' ) );
	...
	private function overview_tab( ImageReport $report, int $attachment_id ): void {
		...
		echo '<table class="janitorix-evidence">';
		$this->row( __( 'Attachment ID', 'janitorix-media-audit' ), '#' . $attachment_id );
		$this->row( __( 'File', 'janitorix-media-audit' ), $report->filename );
		$this->row( __( 'Dimensions', 'janitorix-media-audit' ), isset( $meta['width'] ) ? $meta['width'] . ' × ' . $meta['height'] : '—' );
		$this->row( __( 'Uploaded', 'janitorix-media-audit' ), $date ? $date : '—' );
		$this->row( __( 'Status', 'janitorix-media-audit' ), ucfirst( str_replace( '_', ' ', $report->status ) ) );
		echo '</table>';

		printf(
			'<p><a class="button" href="%s">%s</a></p>',
			esc_url( $link ? $link : '#' ),
			esc_html__( 'Open in Media Library', 'janitorix-media-audit' )
		);
	}
	...
	private function usage_tab( ImageReport $report ): void {
		if ( ! $report->has_evidence() ) {
			echo '<p>' . esc_html__( 'No references were found anywhere on the site.', 'janitorix-media-audit' ) . '</p>';
			echo '<p>' . esc_html__( 'An empty Usage tab is a finding, not a blank page — it is the reason this image is a deletion candidate.', 'janitorix-media-audit' ) . '</p>';
		...
		foreach ( $by_scanner as $scanner => $references ) {
			printf(
				...
				esc_html(
					sprintf(
						...
						_n( '%1$s — %2$d location', '%1$s — %2$d locations', count( $references ), 'janitorix-media-audit' ),
			...
			echo '<table class="janitorix-evidence"><thead><tr>';
			echo '<th>' . esc_html__( 'Where', 'janitorix-media-audit' ) . '</th>';
			echo '<th>' . esc_html__( 'Field', 'janitorix-media-audit' ) . '</th>';
			echo '<th>' . esc_html__( 'What was there', 'janitorix-media-audit' ) . '</th>';
			echo '<th>' . esc_html__( 'How', 'janitorix-media-audit' ) . '</th>';
			echo '<th style="width:90px">' . esc_html__( 'Strength', 'janitorix-media-audit' ) . '</th>';
			echo '</tr></thead><tbody>';
			...
			foreach ( $references as $reference ) {
				...
				printf(
					...
					'' !== $reference->raw_match
						...
						: '<span class="janitorix-t">&mdash;</span>'
				);

				printf( '<td>%s</td>', esc_html( str_replace( '_', ' ', $reference->detection_method ) ) );
				printf( '<td>%d</td>', absint( $reference->strength() ) );
				echo '</tr>';
			}
	...
	private function analysis_tab( ImageReport $report, object $row ): void {
		...
		echo '<h3>' . esc_html__( 'Confidence', 'janitorix-media-audit' ) . '</h3>';
		printf( '<p class="janitorix-big">%d%% <span class="janitorix-label">%s</span></p>', absint( $report->confidence ), esc_html( $report->confidence_level ) );
		echo '<ul class="janitorix-reasons">';
			...
			printf(
				...
				esc_html(
					sprintf(
						...
						_n( '%d distinct check was performed.', '%d distinct checks were performed.', $report->checks_performed, 'janitorix-media-audit' ),
		...
		echo '<h3>' . esc_html__( 'Risk', 'janitorix-media-audit' ) . '</h3>';
		printf(
			'<p><span class="janitorix-pill %s">%s</span> — %d/100</p>',
		...
		echo '<h3>' . esc_html__( 'Coverage', 'janitorix-media-audit' ) . '</h3>';
		printf( '<p>%d%%</p>', (int) $report->coverage );

		echo '<p class="janitorix-t">' . esc_html__( 'Confidence and risk are independent numbers. One is how sure we are; the other is what it would cost to be wrong.', 'janitorix-media-audit' ) . '</p>';
	}
	...
	private function search_log( ImageReport $report, object $row ): void {
		...
		echo '<h3>' . esc_html__( 'Where we looked', 'janitorix-media-audit' ) . '</h3>';
		echo '<ul class="janitorix-reasons janitorix-search-log">';

		foreach ( $log as $line ) {
			$gap = in_array( $line['state'], array( 'failed', 'not_implemented' ), true );

			printf(
				'<li><span class="janitorix-mark %s">%s</span> %s%s</li>',
				esc_attr( $gap ? 'janitorix-danger' : ( $line['found'] ? 'janitorix-neutral' : 'janitorix-safe' ) ),
				esc_html( $gap ? '!' : ( $line['found'] ? '×' : '✓' ) ),
				esc_html(
					$line['found']
						/* translators: %s: scanner name, e.g. "Posts" */
						? sprintf( __( 'USED — found in %s', 'janitorix-media-audit' ), $line['label'] )
						/* translators: %s: scanner name, e.g. "Posts" */
						: sprintf( __( 'Not used in %s', 'janitorix-media-audit' ), $line['label'] )
				),
		...
		printf(
			'<li><span class="janitorix-mark %s">%s</span> %s</li>',
			esc_attr( $broken > 0 ? 'janitorix-caution' : 'janitorix-safe' ),
			esc_html( $broken > 0 ? '!' : '✓' ),
			esc_html(
				$broken > 0
					? sprintf(
					...
					: __( 'No broken references', 'janitorix-media-audit' )
			)
		);

		printf(
			'<li><span class="janitorix-mark janitorix-safe">✓</span> %s</li>',
			esc_html(
				sprintf(
					/* translators: 1: coverage percentage, 2: number of checks */
					__( '%1$d%% scan coverage · %2$d checks', 'janitorix-media-audit' ),
					(int) $report->coverage,
	...
	private function risk_breakdown( ImageReport $report ): void {
		...
		echo '<h4>' . esc_html__( 'How this risk score was reached', 'janitorix-media-audit' ) . '</h4>';
		echo '<table class="widefat striped" style="max-width:620px"><thead><tr>';
		printf( '<th>%s</th>', esc_html__( 'Signal', 'janitorix-media-audit' ) );
		printf( '<th style="width:110px">%s</th>', esc_html__( 'Effect', 'janitorix-media-audit' ) );
		printf( '<th style="width:80px">%s</th>', esc_html__( 'Points', 'janitorix-media-audit' ) );
		echo '</tr></thead><tbody>';
		...
		printf(
			...
			esc_html__( 'Final score', 'janitorix-media-audit' ),
			(int) $report->risk
		);
	...
	private function timeline_tab( int $attachment_id, object $row ): void {
		...
		echo '<h3>' . esc_html__( 'History', 'janitorix-media-audit' ) . '</h3>';
		echo '<table class="widefat striped" style="max-width:620px"><tbody>';

		$this->row( __( 'Uploaded', 'janitorix-media-audit' ), $this->when( $post->post_date_gmt ?? '' ) );
		$this->row( __( 'Last modified', 'janitorix-media-audit' ), $this->when( $post->post_modified_gmt ?? '' ) );
		$this->row( __( 'Last scanned', 'janitorix-media-audit' ), $this->when( (string) ( $scan->completed_at ?? '' ) ) );

		$last_used = $repo->last_seen_used( $attachment_id );

		$this->row(
			__( 'Last seen in use', 'janitorix-media-audit' ),
			null === $last_used
				? __( 'no scan has ever found it in use', 'janitorix-media-audit' )
				: $this->when( $last_used )
		);
		...
		echo '<h3>' . esc_html__( 'Removals', 'janitorix-media-audit' ) . '</h3>';

		if ( empty( $history ) ) {
			echo '<p>' . esc_html__( 'This image has never been trashed by the plugin.', 'janitorix-media-audit' ) . '</p>';

			return;
		}

		echo '<table class="widefat striped" style="max-width:620px"><thead><tr>';
		printf( '<th>%s</th>', esc_html__( 'Action', 'janitorix-media-audit' ) );
		printf( '<th>%s</th>', esc_html__( 'When', 'janitorix-media-audit' ) );
		printf( '<th>%s</th>', esc_html__( 'Restored', 'janitorix-media-audit' ) );
		echo '</tr></thead><tbody>';
			...
			printf(
				...
				esc_html(
					$entry->restored_at
						...
						: __( 'still in Trash', 'janitorix-media-audit' )
				)
			);
		}
	...
	private function logs_tab( int $attachment_id ): void {
		...
		echo '<h3>' . esc_html__( 'Events', 'janitorix-media-audit' ) . '</h3>';

		if ( empty( $entries ) ) {
			echo '<p>' . esc_html__( 'Nothing has been recorded about this image. It has not been trashed, restored, or refused.', 'janitorix-media-audit' ) . '</p>';

			return;
		}

		echo '<table class="widefat striped"><thead><tr>';
		printf( '<th style="width:90px">%s</th>', esc_html__( 'Level', 'janitorix-media-audit' ) );
		printf( '<th style="width:120px">%s</th>', esc_html__( 'When', 'janitorix-media-audit' ) );
		printf( '<th style="width:110px">%s</th>', esc_html__( 'Source', 'janitorix-media-audit' ) );
		printf( '<th>%s</th>', esc_html__( 'Event', 'janitorix-media-audit' ) );
		echo '</tr></thead><tbody>';
		...
		foreach ( $entries as $entry ) {
			...
			printf(
				'<tr><td><span class="janitorix-pill %s">%s</span></td><td>%s</td><td>%s</td><td>%s</td></tr>',
		...
		echo '<p class="janitorix-t">' . esc_html__( 'This log is read-only. It records what the plugin did, including what it refused to do.', 'janitorix-media-audit' ) . '</p>';
	}
	...
	private function when( string $gmt ): string {
		if ( '' === $gmt || '0000-00-00 00:00:00' === $gmt ) {
			return __( 'unknown', 'janitorix-media-audit' );
		}
	...
	private function simulation_tab( ImageReport $report, object $row ): void {
		echo '<h3>' . esc_html__( 'If this image were deleted', 'janitorix-media-audit' ) . '</h3>';
		...
		echo '<p>' . esc_html__( 'Generated sizes and any WebP variants are destroyed along with the original.', 'janitorix-media-audit' ) . '</p>';

		echo '<p><em>' . esc_html__( 'Moving an image to Trash is reversible: a restore record is written before anything is touched, so it can be brought back even if WordPress has no media trash configured.', 'janitorix-media-audit' ) . '</em></p>';
	}
	...
	private function staleness_warning( object $row ): void {
		...
		printf(
			...
			esc_html__( 'This preview is out of date.', 'janitorix-media-audit' ),
			esc_html__( 'The site has changed since the scan it was built from, so what is described below may no longer be true. Run a new scan before acting on it — the plugin will refuse the action until you do.', 'janitorix-media-audit' )
		);
	}
