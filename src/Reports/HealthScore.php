...
final class HealthScore {
	...
	public static function calculate( array $facts ): array {
		...
		if ( 0 === $total ) {
			return array(
				'score'   => 0,
				'label'   => __( 'No images', 'janitorix-media-audit' ),
				'reasons' => array( __( 'The media library is empty.', 'janitorix-media-audit' ) ),
			);
		}
		...
		if ( $coverage < ConfidenceEngine::COVERAGE_FLOOR ) {
			$reasons[] = sprintf(
				...
				__( 'Only %1$d%% of the site could be searched — below the %2$d%% floor.', 'janitorix-media-audit' ),
				$coverage,
		...
		if ( $unused_ratio > 0.25 ) {
			$reasons[] = sprintf(
				...
				__( '%d%% of the library appears unused.', 'janitorix-media-audit' ),
		...
		if ( empty( $reasons ) ) {
			$reasons[] = __( 'Full coverage, nothing risky adrift, and little dead weight.', 'janitorix-media-audit' );
		}
	...
	private static function label( int $score ): string {
		if ( $score >= 90 ) {
			return __( 'Excellent', 'janitorix-media-audit' );
		}
		if ( $score >= 75 ) {
			return __( 'Good', 'janitorix-media-audit' );
		}
		if ( $score >= 50 ) {
			return __( 'Fair', 'janitorix-media-audit' );
		}

		return __( 'Needs attention', 'janitorix-media-audit' );
	}
}
