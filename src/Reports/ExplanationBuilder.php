...
final class ExplanationBuilder {
	...
	private function headline( ImageReport $report ): string {
		$status = array(
			ImageReport::STATUS_USED          => __( 'Used', 'janitorix-media-audit' ),
			ImageReport::STATUS_POSSIBLY_USED => __( 'Possibly used', 'janitorix-media-audit' ),
			ImageReport::STATUS_UNUSED        => __( 'Unused', 'janitorix-media-audit' ),
		)[ $report->status ] ?? __( 'Unknown', 'janitorix-media-audit' );

		$risk_level = '' !== $report->risk_level ? $report->risk_level : __( 'unrated', 'janitorix-media-audit' );

		return sprintf(
			/* translators: 1: status such as "Unused", 2: confidence percentage, 3: risk level */
			__( '%1$s · %2$d%% confidence · %3$s risk', 'janitorix-media-audit' ),
			$status,
			$report->confidence,
			$risk_level
		);
	...
	private function confidence_reason( ImageReport $report ): array {
		if ( $report->has_evidence() ) {
			...
			return array(
				sprintf(
					...
					__( 'Strongest evidence scored %d.', 'janitorix-media-audit' ),
					$strongest
				),
				__( 'Confidence in use is the strength of the single best piece of evidence — evidence does not accumulate.', 'janitorix-media-audit' ),
			);
		}

		$reasons = array(
			__( 'No references were detected by any scanner.', 'janitorix-media-audit' ),
			sprintf(
				/* translators: %d: percentage of the site that was searched */
				__( '%d%% of the searchable site was covered.', 'janitorix-media-audit' ),
				$report->coverage
			),
		);
		...
		$reasons[] = __( 'Confidence in absence is capped at 99% — certainty about a negative is not available.', 'janitorix-media-audit' );
	...
	private function search_note( string $state, int $examined ): string {
		switch ( $state ) {
			case 'skipped':
				return __( 'skipped — not present on this site', 'janitorix-media-audit' );
			case 'failed':
				return __( 'FAILED — this part of the site was not searched', 'janitorix-media-audit' );
			case 'not_implemented':
				return __( 'no scanner built for this yet', 'janitorix-media-audit' );
			case 'warning':
				return $examined > 0
					? sprintf(
						/* translators: %s: formatted number of items checked */
						__( '%s checked, with problems', 'janitorix-media-audit' ),
						number_format_i18n( $examined )
					)
					: __( 'completed with problems', 'janitorix-media-audit' );
		}

		return $examined > 0
			? sprintf(
				/* translators: %s: formatted number of items checked */
				__( '%s checked', 'janitorix-media-audit' ),
				number_format_i18n( $examined )
			)
			: '';
	}
	...
	public function lines( ImageReport $report ): array {
		...
		$lines[] = sprintf(
			...
			__( 'Recommendation: %s', 'janitorix-media-audit' ),
		...
		if ( ! empty( $e['evidence'] ) ) {
			$lines[] = '';
			$lines[] = __( 'Found in:', 'janitorix-media-audit' );
			...
			foreach ( $e['evidence'] as $item ) {
				$lines[] = sprintf(
					...
					__( '  ✓ %1$s — %2$s (%3$s, strength %4$d)', 'janitorix-media-audit' ),
					$item['where'],
					$item['how'],
					$item['scanner'],
					$item['strength']
