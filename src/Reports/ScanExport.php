...
final class ScanExport {
	...
	public function send( int $scan_id, string $format ): void {
		...
		if ( 'json' === $format ) {
			// Default flags only. JSON_UNESCAPED_SLASHES would opt out of escaping
			// that wp_json_encode() applies for a reason, and the saving is one
			// backslash per URL in a file nobody reads by hand.
			echo wp_json_encode( $this->as_array( $scan_id ), JSON_PRETTY_PRINT );
			exit;
		}
	...
	private function as_csv( int $scan_id ): void {
		...
		fputcsv(
			$out,
			array(
				__( 'Attachment ID', 'janitorix-media-audit' ),
				__( 'Filename', 'janitorix-media-audit' ),
				__( 'Status', 'janitorix-media-audit' ),
				__( 'Confidence', 'janitorix-media-audit' ),
				__( 'Risk', 'janitorix-media-audit' ),
				__( 'Risk level', 'janitorix-media-audit' ),
				__( 'Recommendation', 'janitorix-media-audit' ),
				__( 'Size (bytes)', 'janitorix-media-audit' ),
				__( 'Why', 'janitorix-media-audit' ),
			)
		);
