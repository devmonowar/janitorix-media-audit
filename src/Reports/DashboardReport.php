...
final class DashboardReport {
	...
	private function risk_summary( int $scan_id ): array {
		...
		return array(
			...
			'unused_at_risk' => (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM %i WHERE scan_id = %d AND status = 'unused' AND risk >= 60",
					$table,
					$scan_id
				)
			),
			'needs_review'   => (int) $wpdb->get_var(
				$wpdb->prepare(
					"SELECT COUNT(*) FROM %i WHERE scan_id = %d AND recommendation = 'review'",
					$table,
					$scan_id
				)
			),
			'safe'           => (int) $wpdb->get_var(
				$wpdb->prepare(
					'SELECT COUNT(*) FROM %i WHERE scan_id = %d AND risk < 35',
					$table,
					$scan_id
				)
			),
	...
	private function state( object $scan, bool $cache_valid ): string {
		...
		if ( 'running' === $scan->status ) {
			return empty( $scan->resume_state )
				? __( 'Running', 'janitorix-media-audit' )
				: __( 'Paused', 'janitorix-media-audit' );
		}

		return $cache_valid
			? __( 'Completed', 'janitorix-media-audit' )
			: __( 'Needs Rescan', 'janitorix-media-audit' );
	}
	...
	private function notices( object $scan, bool $cache_valid ): array {
		...
		if ( ! $cache_valid ) {
			$notices[] = array(
				'type' => 'warning',
				'text' => __( 'The site has changed since this scan — the results below are out of date. Run a new scan.', 'janitorix-media-audit' ),
			);
		}
		...
		if ( (int) $scan->coverage < ConfidenceEngine::COVERAGE_FLOOR ) {
			$notices[] = array(
				...
				'text' => sprintf(
					...
					__( 'Coverage is %1$d%%, below the %2$d%% floor. No deletion is recommended at any confidence until this is fixed.', 'janitorix-media-audit' ),
					$scan->coverage,
		...
		if ( ! empty( $summary['failed'] ) ) {
			$notices[] = array(
				...
				'text' => sprintf(
					...
					__( 'These scanners failed: %s. Their part of the site was not searched.', 'janitorix-media-audit' ),
