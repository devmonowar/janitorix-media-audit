...
namespace JanitorixMediaAudit\Calibration;
...
final class CalibrationRun {
	...
	private function seed(): array {
		...
		$logo                        = new Fixture( 'ghost logo in a theme option', 'janitorix-cal-site-logo.png' );
		...
		$logo->min_risk              = 60;
		$logo->attachment_id         = $this->seeder->create_image( 'janitorix-cal-site-logo.png', 240, 60 );
		...
		$this->seeder->set_option(
			'janitorix_calibration_unknown_theme_settings',
		...
		// same as correct.
		$orphan                        = new Fixture( 'genuine orphan', 'janitorix-cal-IMG_4821.png' );
		...
		$orphan->max_risk              = 14;
		$orphan->attachment_id         = $this->seeder->create_image( 'janitorix-cal-IMG_4821.png', 3000, 2000 );
		$fixtures[]                    = $orphan;

		// 3. Evidence beats profile. A file named like site furniture, but
		// genuinely used inside a published post. Formula A, not B.
		$example                        = new Fixture( 'logo-examples.png used in a post', 'janitorix-cal-logo-examples.png' );
		...
		$example->attachment_id         = $this->seeder->create_image( 'janitorix-cal-logo-examples.png', 900, 600 );

		$this->seeder->create_post(
			'Janitorix calibration — a post about brand design',
			sprintf(
		...
		$svg                        = new Fixture( 'unattached SVG with alt text', 'janitorix-cal-graphic.svg' );
