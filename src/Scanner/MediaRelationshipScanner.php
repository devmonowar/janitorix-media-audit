...
final class MediaRelationshipScanner implements Scanner {
	...
	private function post_parents( AttachmentResolver $resolver ): ?array {
		...
		foreach ( array_chunk( array_keys( $rendering ), self::BATCH_SIZE ) as $parent_ids ) {
			...
			// phpcs:disable WordPress.DB.PreparedSQLPlaceholders, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- 'image/%%' is a literal MIME prefix, no variable involved; the sniff also cannot see into the dynamic %d count in the IN() clause (verified count($parent_ids) always matches the placeholders built above).
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT a.ID AS attachment_id, parent.ID AS post_id, parent.post_title, parent.post_status
					 FROM %i a
					 INNER JOIN %i parent ON parent.ID = a.post_parent
					 WHERE a.post_type = 'attachment'
					   AND a.post_mime_type LIKE 'image/%%'
					   AND a.post_parent IN ( {$placeholders} )",
					array_merge( array( $wpdb->posts, $wpdb->posts ), $parent_ids )
				)
			);
