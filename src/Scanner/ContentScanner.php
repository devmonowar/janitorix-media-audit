...
namespace JanitorixMediaAudit\Scanner;

use JanitorixMediaAudit\Scanner\Contracts\Scanner;
...
final class ContentScanner implements Scanner {
	...
	public function scan( AttachmentResolver $resolver ): ScannerResult {
		...
		do {
			...
			// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- the two interpolated variables are runs of literal '%s' built by array_fill(), one per post type and per status, and every one of them is filled from the merged args of this same prepare() call. They carry no data; the sniff counts only placeholders written literally.
			$sql = $wpdb->prepare(
				"SELECT ID, post_title, post_type, post_status, post_content
				 ...
				 ORDER BY ID ASC
				 LIMIT %d OFFSET %d",
				array_merge( array( $wpdb->posts ), $types, $statuses, array( self::BATCH_SIZE, $offset ) )
			);
		...
		if ( ! empty( $unexpanded ) ) {
			...
			$warnings[] = sprintf(
				...
				__( 'Content uses shortcodes no plugin on this site registers, so what they render could not be read: %s.', 'janitorix-media-audit' ),
	...
	private function post_types(): array {
		...
		return array_values( (array) apply_filters( 'janitorix_content_scanner_post_types', $types ) );
	}
