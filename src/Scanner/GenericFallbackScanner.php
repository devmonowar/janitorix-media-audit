...
final class GenericFallbackScanner implements Scanner {
	...
	private function sweep_urls( AttachmentResolver $resolver ): int {
		...
		// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.LikeWildcardsInQuery -- every pattern here is a literal, no variable involved; already correctly double-escaped.
	...
	private function sweep_structures( AttachmentResolver $resolver ): int {
		...
		do {
			// phpcs:disable WordPress.DB.PreparedSQLPlaceholders, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- the three LIKE patterns are literal, no variable involved, already correctly double-escaped; the sniff also cannot see into the dynamic %s count in the NOT IN() clause (verified count($excluded) always matches the placeholders built above).
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT post_id, meta_key, meta_value
					 FROM %i
					 ...
					 ORDER BY meta_id ASC
					 LIMIT %d OFFSET %d",
					array_merge( array( $wpdb->postmeta ), $excluded, array( self::BATCH_SIZE, $offset ) )
				)
			);
