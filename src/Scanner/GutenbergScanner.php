	...
	public function scan( AttachmentResolver $resolver ): ScannerResult {
		...
		do {
			$rows = $wpdb->get_results(
				// phpcs:disable WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- the sniff cannot see into $placeholders' dynamic %s count; verified it always matches count( $types ).
				$wpdb->prepare(
					"SELECT ID, post_title, post_type, post_content
					 ...
					 ORDER BY ID ASC
					 LIMIT %d OFFSET %d",
					array_merge( array( $wpdb->posts ), $types, array( self::BATCH_SIZE, $offset ) )
				)
			);
