 ...
 * invents its own option keys. `janitorix_image_key_hints` is that extension point.
 *
...
use JanitorixMediaAudit\Scanner\ImageValueExtractor;
...
function hints_with( ?callable $filter ): array {
	...
	$GLOBALS['janitorix_test_filters'] = array();

	if ( null !== $filter ) {
		$GLOBALS['janitorix_test_filters']['janitorix_image_key_hints'] = $filter;
	}
