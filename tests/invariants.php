...
echo "Janitorix Media Audit — safety invariants\n";
echo str_repeat( '-', 60 ) . "\n";
...
foreach ( php_files( $src . '/Scanner' ) as $file ) {
	...
	if ( preg_match( '/use\s+JanitorixMediaAudit\\\\(Risk|Recommendation|Safety|Cleanup)\\\\/', $source ) ) {
...
foreach ( $all_files as $file ) {
	...
	$expected = 'JanitorixMediaAudit\\' . str_replace( '/', '\\', dirname( rel( $root, $file ) ) );
	$expected = rtrim( str_replace( 'JanitorixMediaAudit\\src', 'JanitorixMediaAudit', $expected ), '\\' );
