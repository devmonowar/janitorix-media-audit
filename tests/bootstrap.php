 ...
 */
$GLOBALS['janitorix_test_options'] = $GLOBALS['janitorix_test_options'] ?? array();
 ...
 */
$GLOBALS['janitorix_test_filters'] = $GLOBALS['janitorix_test_filters'] ?? array();
...
if ( ! function_exists( 'get_option' ) ) {
	...
	function get_option( string $name, $default = false ) { // phpcs:ignore
		return $GLOBALS['janitorix_test_options'][ $name ] ?? $default;
	}
}
...
if ( ! function_exists( 'get_post' ) ) {
	 ...
	 * @return WP_Post|null
	 */
	function get_post( $post = 0, $output = '', $filter = 'raw' ) { // phpcs:ignore
		return null;
	}
}
...
if ( ! function_exists( 'apply_filters' ) ) {
	/**
	 * A test registers a hook by assigning `$GLOBALS['janitorix_test_filters'][$hook]`
	...
	function apply_filters( string $hook, $value ) { // phpcs:ignore
		$filter = $GLOBALS['janitorix_test_filters'][ $hook ] ?? null;
