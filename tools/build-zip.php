 ...
 *
 * Output: build/janitorix-media-audit.zip (both `build/` and `*.zip` are
 ...
 */
function janitorix_is_ignored( string $relative_path, array $ignore ): bool {
...
$staging_dir = rtrim( sys_get_temp_dir(), '/\\' ) . '/janitorix-build-' . $slug;
$zip_path    = $build_dir . '/' . $slug . '.zip';

/** Recursively delete a directory. */
function janitorix_rrmdir( string $dir ): void {
	if ( ! is_dir( $dir ) ) {
		return;
	}
	$items = scandir( $dir );
	foreach ( $items as $item ) {
		...
		$path = $dir . '/' . $item;
		is_dir( $path ) ? janitorix_rrmdir( $path ) : unlink( $path );
	}
	rmdir( $dir );
}

janitorix_rrmdir( $staging_dir );
mkdir( $staging_dir, 0777, true );

/** Recursively copy `$src` into `$dest`, skipping ignored paths. */
function janitorix_copy_tree( string $src, string $dest, string $repo_root, array $ignore ): int {
	$copied = 0;
	$items  = scandir( $src );
	foreach ( $items as $item ) {
		if ( '.' === $item || '..' === $item ) {
		...
		if ( janitorix_is_ignored( $relative_path, $ignore ) ) {
			continue;
		}
		...
		if ( is_dir( $src_path ) ) {
			...
			$copied += janitorix_copy_tree( $src_path, $dest_path, $repo_root, $ignore );
		} else {
...
$file_count = janitorix_copy_tree( $root, $staging_dir, $root, $ignore );
