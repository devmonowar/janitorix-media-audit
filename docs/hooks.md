# Janitorix Media Audit — Developer Hooks

Janitorix Media Audit is built to be extended. A handful of hooks let you widen what the scan
looks at or add an entirely new scanner of your own — no core edits required.

All hooks are prefixed `janitorix_`.
...
- `ScannerRegistry $registry` — call `$registry->add( $scanner )` with an instance implementing
  `JanitorixMediaAudit\Scanner\Contracts\Scanner` (`id()`, `label()`, `is_applicable()`,
...
```php
add_action( 'janitorix_register_scanners', function ( $registry ) {
...
```php
add_filter( 'janitorix_image_key_hints', function ( $hints ) {
...
## `janitorix_content_scanner_post_types`
...
```php
add_filter( 'janitorix_content_scanner_post_types', function ( $types ) {
...
[GitHub](https://github.com/devmonowar/janitorix-media-audit).
