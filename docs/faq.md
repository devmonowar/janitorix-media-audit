...
If a row shows **Protected** instead of a checkbox, there is always a specific, statable reason, and
the screen tells you it: a note above the table lists the reasons for that page, hovering the label
shows the one for that image, and opening the image spells it out in full. See
[Staying Safe](staying-safe.md) for the rules themselves.

The one that surprises people most: an image uploaded in the last 24 hours is held back on purpose,
because it may not have been placed on a page yet. If you uploaded a picture specifically to try the
plugin on, that is what you are seeing — not a scan that missed it.
