...
blank — that's this behaviour working as intended, not an error. The screen says which rule is
holding it: a note above the table groups the reasons for that page, and hovering the label shows the
same sentence for that one image.

This is worth knowing if you're trying the plugin out on an image you just uploaded: it will be held
back for a day, and the plugin will tell you that is why.
